...
if len(sys.argv) == 1:
    print(
        "Usage: {} invocation-track-id path/to/first/toolpackage path/to/second/toolpackage/ path/to/third/toolpackage ...".format(
            sys.argv[0]
        )
    )
    exit(1)
        ...
        if not os.path.isfile(settings.invocations_filename_name()):
            print(
                "ERROR: Invocations file {} does not exist".format(
                    settings.invocations_filename_name()
                )
            )
        ...
        if not os.path.isfile(settings.toolscript_filename_name()):
            print(
                "ERROR: Tool script file {} does not exist".format(
                    settings.toolscript_filename_name()
                )
            )
            raise AssertionError(
...
for tool in tools:
    ...
    if not os.path.isdir(tooldir):
        raise AssertionError(
            "Unable to find directory {}".format(os.path.join(os.path.curdir, tooldir))
        )
...
if not os.path.exists(output_dir):
    raise AssertionError("Output directory {} does not exist".format(output_dir))
...
for benchmark_csv in benchmarks_csv:
    benchmark_ids.append(
        "{}.{}.{}".format(benchmark_csv[0], benchmark_csv[3], benchmark_csv[4])
    )
...
def create_log_page(toolname, benchmark_identifier, result_json):
    ...
    f_path = os.path.join(
        "logs/" + toolname, os.path.basename(logfilepath)[:-4] + ".html"
    )
    with open(os.path.join(output_dir, f_path), "w") as f:
        ...
        write_line(
            f,
            indention,
            '<tr><td>Model:</td><td><a href="{}">{}</a> <span class="tt">v.{}</span> ({})</td></tr>'.format(
                "../../../../../benchmarks/index.html#{}".format(
                    b.get_model_short_name()
                ),
                b.get_model_short_name(),
                b.index_json["version"],
                b.get_model_type().upper(),
            ),
        )
        write_line(
            f,
            indention,
            "<tr><td>Parameter(s)</td><td>{}</td></tr>".format(
                ", ".join(
                    [
                        '<span class="tt">{}</span> = {}'.format(p["name"], p["value"])
                        for p in b.get_parameters()
                    ]
                )
            ),
        )
        write_line(
            f,
            indention,
            '<tr><td>Property:</td><td><span class="tt">{}</span> ({})</td></tr>'.format(
                b.get_property_name(), b.get_property_type()
            ),
        )
        indention -= 1
        ...
        write_line(
            f,
            indention,
            '<div class="boxlabelo"><div class="boxlabelc">Invocation ({})</div></div>'.format(
                result_json["invocation-id"]
            ),
        )
        f.write(
            "\t" * indention
            + '<pre style="overflow: auto; padding-bottom: 1.5ex; margin-bottom: 0ex;  margin-top: 0ex;">'
        )
        ...
        if result_json["timeout"]:
            write_line(
                f,
                indention,
                '<tr><td>Walltime:</td><td style="color: red;">&gt {}s (Timeout)</td></tr>'.format(
                    settings.time_limit()
                ),
            )
        else:
            write_line(
                f,
                indention,
                '<tr><td>Walltime:</td><td style="tt">{}s</td></tr>'.format(
            ...
            if result_json["execution-error"]:
                write_line(
                    f,
                    indention,
                    '<tr><td>Return code:</td><td style="tt; color: red;">{}</td></tr>'.format(
                        ", ".join(return_codes)
                    ),
                )
            else:
                write_line(
                    f,
                    indention,
                    '<tr><td>Return code:</td><td style="tt">{}</td></tr>'.format(
                        ", ".join(return_codes)
                    ),
                )
        first = True
        ...
        if "relative-error" in result_json:
            write_line(
                f,
                indention,
                '<tr><td>Relative Error:</td><td style="tt{}">{}</td></tr>'.format(
                    "" if result_json["result-correct"] else "; color: red",
                    result_json["relative-error"],
                ),
            )
        indention -= 1
        write_line(f, indention, "</table>")
        ...
        for log in logs:
            ...
            if pos >= 0:
                ...
                write_line(
                    f,
                    indention,
                    '<div class="boxlabelo"><div class="boxlabelc">STDERR</div></div>',
                )
                f.write(
                    "\t" * indention
                    + '<pre style="overflow:auto; padding-bottom: 1.5ex">'
...
def get_result_str(toolname, benchmark_identifier):
    ...
    for res in all_results:
        if (
            res["benchmark-id"] == benchmark_identifier
            and res["invocation-track-id"] == track_id
        ):
            results.append(res)
    if len(results) == 0:
        return ""
        ...
        for res in results:
            if res["invocation-id"] == invid:
                ...
                if res["timeout"]:
                    ...
                    "result" in res
                    and "result-correct" in res
                    and not res["result-correct"]
                ):
                    res_str = "INC"
                    ...
                    if not (track_id == "often-epsilon-correct-10-min"):
                        ...
                        tool_times[benchmark_identifier][toolname].append(
                            res["wallclock-time"]
                        )
                    else:
                        if "relative-error" in res:
                            ...
                            benchmark = get_benchmark_from_id(
                                settings, res["benchmark-id"]
                            )
                            if (
                                benchmark.has_reference_result()
                            ):  # result is a boolean value
                                correct = is_result_correct(
                                    settings,
                                    benchmark.get_reference_result(),
                                    try_to_bool_or_number(res["result"]),
                                ...
                                print(
                                    res["benchmark-id"]
                                )  # check these results by hand, there is no reference result
                ...
                results_str.append(
                    "<a href='{}' {}>{}</a>".format(logpage, link_attributes, res_str)
                )
    ...
    for inv in invocations:
        if (
            inv["benchmark-id"] == b.get_identifier()
            and len(inv["commands"]) > 0
            and inv["commands"][0].strip() != ""
        ):
            inv_ids.append(inv["invocation-id"])
    if (
        inv_ids != []
        and inv_ids != ["default"]
        and inv_ids != ["default", "specific"]
        and inv_ids != ["specific", "default"]
    ):
        print(
            "Invalid inv ids {} for {} / {}".format(inv_ids, tool, b.get_identifier())
        )
...
def create_quantile_plot_csv(tool_subset=tools, n=None, default_times=True):
    ...
    for id in benchmark_ids:
        ...
        if num_supported_tools >= n or is_supported_by_comp_tool:
            ...
            for tool in tool_subset:
                if len(supported_tools[id][tool]) > 0:
                    if supported_tools[id][tool][results_index] not in [
                        "TO",
                        "ERR",
                        "INC",
                    ]:
    ...
    header = ["n"]
    for tool in tool_subset:
        header += [tool, tool + "scaled", tool + "shifted"]
    result_csv = [header]
    ...
    caption = "Quantile plot for {} benchmarks: {}.".format(
        num_selected_benchmarks,
        ", ".join(
            [
                "{}({}/{})".format(
                    tool, len(sorted_times[tool]), num_supported_benchmarks[tool]
                )
                for tool in tool_subset
            ]
        ),
    ...
    for id in benchmark_ids:
        if len(supported_tools[id][tool]) > 0:
            ...
            for compare_tool in tools:
                if (
                    compare_tool == tool or compare_tool == "Storm"
                ):  # do not compare with Storm
                    continue
                if (
                    tool == "Storm" and compare_tool == "Storm-static"
                ):  # do not compare Storm with Storm-static
                    continue
                if len(supported_tools[id][compare_tool]) > 0 and supported_tools[id][
                    compare_tool
                ][results_index] not in {"TO", "ERR", "INC"}:
                    compare_value = min(
                        tool_times[id][compare_tool][results_index], compare_value
                    )
...
def create_scatter_plot_vs_all_csv_relative_error(tool, default_times=True):
    ...
    for id in benchmark_ids:
        if len(supported_tools[id][tool]) > 0:
            ...
            if (
                compare_value <= 0
            ):  # if relative-error is 0.0 write 1e-10 into csv because of logscale in plots
                row.append(1e-20)
            else:
                row.append(compare_value)
            row.append(max(1, compare_value))
...
"""
    ...
    for benchmark_id in benchmark_ids:
        ...
        write_line(
            ...
            '<td><a href="{}">{}</a></td>'.format(
                os.path.join(
                    qcomp_root,
                    "benchmarks/index.html#{}".format(b.get_model_short_name()),
                ),
                b.get_model_short_name(),
            ),
        )  # for website: "benchmarks/index.html#{}".format
        write_line(
            tablefile, indention, "<td>{}</td>".format(b.get_model_type().upper())
        )
        write_line(tablefile, indention, "<td>{}</td>".format(b.get_original_format()))
        write_line(
            tablefile, indention, "<td>{}</td>".format(b.get_parameter_values_string())
        )
        write_line(tablefile, indention, "<td>{}</td>".format(b.get_property_name()))
        write_line(
            tablefile, indention, "<td>{}</td>".format(b.get_short_property_type())
        )
        for toolname in toolnames:
            write_line(
                tablefile,
                indention,
                "<td>{}</td>".format(get_result_str(toolname, b.get_identifier())),
            )
        indention -= 1
        write_line(tablefile, indention, "</tr>")
    indention -= 1
    ...
    for text, show, hide in zip(
        ["Show all", "Hide all"], [tool_columns, []], [[], tool_columns]
    ):
        write_line(tablefile, indention, "{")
        indention += 1
    ...
    indention += 1
    write_line(
        tablefile, indention, 'table.on("column-sizing.dt", function (e, settings) {'
    )
    indention += 1
...
with open(os.path.join(output_dir, "style.css"), "w") as stylefile:
    write_line(
        ...
        0,
        '@import url("{}");'.format(
            os.path.join(qcomp_root, "fonts/Tajawal/Tajawal.css")
        ),
    )
    stylefile.write(
        r"""
    ...
    \begin{document}"""
        ...
        for tool in toolnames:
            for default in [True, False]:
                ...
                cap = "Scatter Plot for {} with {} settings (n={}).".format(
                    tool, "default" if default else "specific", len(csv) - 1
                )
                plotfile.write(
                    r"""
            \begin{center}
                ...
                \scatterplot{"""
                    ...
                    + (
                        " ({}, fastest on {}/{})".format(
                            "default" if default else "specific",
                            num_fastest,
                            len(csv) - 1,
                        )
                    )
        ...
        for default in [True, False]:
            ...
            filename = "scatter_storm_vs_static{}.csv".format(
                "" if default else "specific"
            )
            num_fastest = 0
            for row in csv[1:]:
            ...
            save_csv(csv, os.path.join(plot_dir, filename), delim=";")
            cap = (
                "Scatter Plot for {} vs. Storm-static with {} settings (n={}).".format(
                    tool, "default" if default else "specific", len(csv) - 1
                )
            )
            plotfile.write(
        ...
        \begin{center}
            ...
            \scatterplotstorm{"""
                ...
                + (
                    " ({}, fastest on {}/{})".format(
                        "default" if default else "specific", num_fastest, len(csv) - 1
                    )
                )
                + "}{legend}"
                + r"""
